  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden bg-white">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
        <div className="w-3 h-3 rounded-sm border border-gray-200" style={{ backgroundColor: unitColor }} />
        ...
        {existing && (
          <button onClick={() => removeDesignLine(unitId)}
            className="ml-auto text-red-400 hover:text-red-600 text-xs cursor-pointer">Remove</button>
        )}
      </div>
      ...
      <div className="px-3 py-2 space-y-3">
        ...
        <button onClick={save}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs py-1.5 rounded cursor-pointer">
...
export function DesignLinePanel() {
  ...
  return (
    <div className="space-y-3 overflow-y-auto h-full pr-1">
      <p className="text-xs text-gray-500">
        Define a design line per geological unit: constant value, segmented step function, or Python equation.
      </p>
      {units.map((u) => (
