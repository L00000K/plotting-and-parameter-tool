    ...
    <div className="border border-slate-700 rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: unit.color }} />
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={(e) => { e.stopPropagation(); removeGeoUnit(unit.id); }}
      ...
      {open && (
        <div className="px-3 pb-3 pt-1 grid grid-cols-2 gap-x-3 gap-y-2">
          <Field label="Name" value={unit.name} type="text"
            onChange={(v) => update('name', v)} />
          <Field label="Color" value={unit.color} type="color"
...
function Field({
  label, value, type, step, onChange,
}: {
  label: string;
  value: string | number;
  type: string;
  ...
  return (
    <label className="text-xs text-slate-400">
      ...
      <input
        ...
        className="mt-0.5 block w-full bg-slate-800 border border-slate-600 rounded px-2 py-1 text-slate-200 text-xs"
      />
    </label>
  );
}
