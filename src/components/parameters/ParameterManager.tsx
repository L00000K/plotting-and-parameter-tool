  ...
  return (
    <div className="space-y-4 overflow-y-auto h-full pr-1">
      <div className="border border-gray-200 rounded-lg p-3 space-y-2 bg-white">
        <h3 className="text-sm font-semibold text-gray-700">Site Parameters</h3>
        <div className="grid grid-cols-2 gap-3">
          <label className="text-xs text-gray-600">
            Ground Level (m AOD)
            <input type="number" step="0.1" value={ground_model.ground_level}
              onChange={(e) => setGroundLevel(parseFloat(e.target.value) || 0)}
              className="mt-1 block w-full bg-white border border-gray-300 rounded px-2 py-1 text-gray-800 text-xs focus:outline-none focus:border-blue-400" />
          </label>
          <label className="text-xs text-gray-600">
            GWT Elevation (m AOD)
            <input type="number" step="0.1" value={ground_model.gwt_elevation}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-700">Geological Units</h3>
          <button
            onClick={() => addGeoUnit({
              ...
              gamma: 19, gamma_sat: 20, phi_cv: 33,
            })}
            className="text-xs bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded cursor-pointer"
          >
            + Add Unit
          </button>
        </div>
